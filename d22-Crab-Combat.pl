...
# useful modules
use List::Util qw/sum any all max/;
...
sub recurse_game {
    ...
    my %states;
    $game_no++;
    print "Starting game $game_no at depth $depth\r" if ( $game_no % 100 == 0);
...
while (1) {
    say "==> $round";
    for (0,1) {
	say join(' ', @{$players[$_]->{deck}});
    }
    if (all { scalar $players[$_]->{deck}->@* >1 } (0,1) ) { # both have cards
	my $d1 = shift @{$players[0]->{deck}};
	my $d2 = shift @{$players[1]->{deck}};
	if ($d1 > $d2 ) {
	    say "    player 1 wins!";
	    push @{$players[0]->{deck}}, ( $d1, $d2 );
	} elsif ($d2> $d1) {
	    say "    player 2 wins!";
	    push @{$players[1]->{deck}}, ( $d2, $d1 );
	} else {
	    die "equal cards $d1 and $d2 - someone's cheating!!";
	}
    } elsif (any {scalar $players[$_]->{deck}->@*  == 1} (0,1)) {
	# final round
	my $d1 = shift @{$players[0]->{deck}};
	my $d2 = shift @{$players[1]->{deck}};
	if ($d1 > $d2 ) {
...
say dump \@players;
foreach (0,1) {
    next if scalar @players[$_]->{deck} == 0;
    my $factor = 1;
    my $sum = 0;
    foreach (reverse $players[$_]->{deck}->@*) {
	$sum += $factor * $_;
	$factor++;
    }
    say $sum;
